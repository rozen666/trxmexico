...
class product_product(models.Model):
    ...
    sku_product = fields.Char(string='SKU', index=True)
    check_visible = fields.Boolean(String='Descontinuar Producto')
...
class account_invoice(models.Model):
	...
	def _compute_amount(self):
		if self.type_cotizacion == False:				
			if self.check_envio:
				if self.descuento_habilitar =='2':
					self.subtotal_con_desc = sum(line.price_subtotal for line in self.invoice_line)
				else:
			...
			self.amount_tax = (self.subtotal_con_desc *0.16) + (self.costo_envio*0.16)
			self.amount_envio = self.costo_envio
			# self.amount_tax = sum(line.amount for line in self.tax_line) + (self.costo_envio*0.16)
			self.total_con_desc = self.subtotal_con_desc + self.amount_tax + self.costo_envio
			self.amount_untaxed = sum(line.price_subtotal for line in self.invoice_line)
			dolar_id = self.env['trxmexico.change.monetary'].search([('id', '=', 1)])
			mxn = (self.subtotal_con_desc + self.amount_tax + self.costo_envio) * float(dolar_id.price_dolar)
			...
			dolar_id = self.env['trxmexico.change.monetary'].search([('id', '=', 1)])
			mxn = (self.subtotal_con_desc + self.amount_tax + self.costo_envio)
			self.amount_tax_mxn =  mxn

	# Compute para obtner la empresa de cada cliente
	@api.one
	@api.depends('partner_id')
	def _compute_empresa(self):
		self.empresa_partner = self.partner_id.function
	...
	observaciones = fields.Text(string='Observaciones', index=True)
	empresa_partner = fields.Char(string='Empresa', compute='_compute_empresa',  store=True, readonly=True)
	proyecto_name = fields.Char(string='Proyecto')
	amount_envio = fields.Float(string='Costo Envio', compute='_compute_amount', store=True)
	...
	def create(self,values):
		if 'in_context' in self._context:
			...
			records = len(self.search([("type_cotizacion","!=",False)]))+1
		else:
			records = len(self.search([("type_cotizacion","=",False)]))+1
	...
	# 	# Crea el codígo con el subfijo y la nomenclatura
		...
		code = 'CTZ-PPM'+'-'+anio+'-'+str(records).zfill(5)
	...
	# 	vals['total_con_desc'] = float(invoice.costo_envio) + float(invoice.subtotal_con_desc) + float(invoice.amount_tax)
	# 	# self.button_reset_taxes_trx(cr, uid, ids)
	# 	new_id =super(account_invoice, self).write(cr,uid, ids, vals, context=context)
	# 	return new_id

	# Boton que permite actualizar los impuestos segun el tipo de Descuento (field->descuento_habilitar)
	@api.onchange('invoice_id','invoice_id_entrega','invoice_id_general','costo_envio','invoice_line')
	def _onchange_condiciones(self):
		...
		# total_cd = self.subtotal_con_desc + self.amount_tax + self.amount_envio
		self.button_reset_taxes_trx()
		self.total_con_desc  = self.subtotal_con_desc + self.amount_tax + self.amount_envio
		...
		for invoice in self:
			...
			if invoice.descuento_habilitar =='0':
				...
				for taxed in self.env['account.invoice.line'].search([('invoice_id', '=', invoice.id)]):
					...
					taxe = {
						...
						'manual': False,
						'base_amount':float(taxed.price_subtotal),
						...
						'base': float(taxed.price_subtotal),
			...
			if invoice.descuento_habilitar =='1':
				...
				for taxed in self.env['account.invoice.line'].search([('invoice_id', '=', invoice.id)]):
					...
					price_subtotal = float(taxed.price_subtotal)
				...
				invoice.write({'des_subtotal': desc_total,})
			...
			if invoice.descuento_habilitar =='2':
				...
				for taxed in self.env['account.invoice.line'].search([('invoice_id', '=', invoice.id)]):
					...
					if 'in_context' in self._context:
						...
						if taxed.descuento != 0 and taxed.modificar_precio==False:
							price_subtotal_s = (float(taxed.product_id.price_sell_mxn)/1.16) * float(float(taxed.descuento)/100)
							price_subtotal = (float(taxed.product_id.price_sell_mxn)/1.16) - float(price_subtotal_s)
							# descuento = float(descuento) + float(price_subtotal_s)
							descuento = float(descuento) + float(price_subtotal_s) * float(float(taxed.quantity))

						elif taxed.descuento != 0 and taxed.modificar_precio==True:
							price_subtotal_s = (float(taxed.price_unit)/1.16) * float(float(taxed.descuento)/100)
							price_subtotal = (float(taxed.price_unit)/1.16) - float(price_subtotal_s)
							# descuento = float(descuento) + float(price_subtotal_s)
							descuento = float(descuento) + float(price_subtotal_s) * float(float(taxed.quantity))
						
						else:
							price_subtotal = float(taxed.product_id.price_sell_mxn)/1.16
					else:
						# Si hay conceptos con descuento se realiza la operación para obtner el precio con el descuento de lo contrario se asigna el valor original de cada producto
						if taxed.descuento != 0 and taxed.modificar_precio==False:
							price_subtotal_s = (float(taxed.product_id.lst_price)/1.16) * float(float(taxed.descuento)/100)
							price_subtotal = (float(taxed.product_id.lst_price)/1.16) - float(price_subtotal_s)
							# descuento = float(descuento) + float(price_subtotal_s)
							descuento = float(descuento) + float(price_subtotal_s) * float(float(taxed.quantity))
							...
							price_subtotal = float(taxed.product_id.lst_price)/1.16
	...
	def enviar_cotizacion_partner(self, cr, uid,ids, context=None):
		...
		report_id = self.pool.get('ir.actions.report.xml').search(cr, uid, [('name','=','COTIZACION2'), ('report_name', '=', 'COTIZACION2')])[0]
...
class account_invoice_line(models.Model):
	...
	def _compute_description_p(self):
		# Obtnemos la descripción de producto
		self.description_p = self.product_id.description
	
	#Campos TRXMexico account_invoice_line
	price_sell_mxn = fields.Float(string='Precio MXN', index=True, compute='_compute_amount_mxn', store='True')
	amount_mxn = fields.Float(string='Monto MXN', index=True, compute='_compute_amount_mxn', store='True')
	descuento = fields.Char(string='Descuento %', size=4)
	modificar_precio = fields.Boolean(string='Modificar Precio')
	description_p = fields.Char(string='Descipción', compute='_compute_description_p', store='True')

	#Campo para ver el precio real del producto Update M1
	precio_pro = fields.Float(string='Precio Unit', compute='_compute_precio', store='True')

	# compute para obtener el precio del prodcuto sin modificarlo
	@api.multi
	@api.depends('product_id')
	def _compute_precio(self):
		for row in self:
			if 'in_dolars' in self._context:
				row.precio_pro = float(row.product_id.list_price)/1.16
			else:
				row.precio_pro = float(row.product_id.price_sell_mxn)/1.16
